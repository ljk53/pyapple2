...
class ObservableMemoryTests(unittest.TestCase):
    ...
    def test_write_tracking_disabled_by_default(self):
        mem = ObservableMemory(subject=self._make_subject())
        self.assertIsNone(mem.get_write_counts())

    def test_write_tracking_enable(self):
        mem = ObservableMemory(subject=self._make_subject())
        mem.enable_write_tracking()
        wc = mem.get_write_counts()
        self.assertIsNotNone(wc)
        self.assertEqual(len(wc), 0x10000)
        self.assertTrue(all(c == 0 for c in wc))

    def test_write_tracking_increments(self):
        mem = ObservableMemory(subject=self._make_subject())
        mem.enable_write_tracking()
        mem[0x0400] = 0x41
        mem[0x0400] = 0x42
        mem[0x1234] = 0xFF
        wc = mem.get_write_counts()
        self.assertEqual(wc[0x0400], 2)
        self.assertEqual(wc[0x1234], 1)
        self.assertEqual(wc[0x0000], 0)

    def test_write_tracking_saturates_at_255(self):
        mem = ObservableMemory(subject=self._make_subject())
        mem.enable_write_tracking()
        for i in range(300):
            mem[0x0300] = i & 0xFF
        self.assertEqual(mem.get_write_counts()[0x0300], 255)

    def test_write_tracking_clear(self):
        mem = ObservableMemory(subject=self._make_subject())
        mem.enable_write_tracking()
        mem[0x0400] = 0x41
        mem.clear_write_counts()
        self.assertEqual(mem.get_write_counts()[0x0400], 0)

    def test_write_tracking_no_overhead_when_disabled(self):
        mem = ObservableMemory(subject=self._make_subject())
        # Should not raise even without tracking enabled
        mem[0x0400] = 0x41
        self.assertIsNone(mem.get_write_counts())
